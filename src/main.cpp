...
int main() {
  ...
  h.onMessage([&car_lane, &ref_vel, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // variable to save sensor_fusion reading lane based on d-coordinate (fernet)
          	int check_car_lane;
          	...
          	for(unsigned int i = 0; i < sensor_fusion.size(); i++)
            {
              ...
              //if we on same lane and distance less than 30, we can not keep on lane. change to false
